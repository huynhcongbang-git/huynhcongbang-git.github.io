...
Các design pattern khác nhau bởi sự phức tạp của nó, sự chi tiết và khả năng áp dụng của chúng trên toàn bộ hệ thống đang được thiết kế. Tôi thích sự tương đồng với xây dựng con đường: bạn có thể tạo ra một nút giao an toàn hơn bằng việc gắn một vài đèn giao thông hoặc là xây dựng lại toàn bộ nút giao thông đa tầng với đường hầm dành cho người đi bộ.

Các pattern căn bản và cấp thấp nhất thường được gọi là **idioms - thành ngữ**. Chúng thường chỉ được áp dụng cho một ngôn ngữ lập trình duy nhất.

Các pattern phổ biến và cấp cao nhất thường gọi là **architectural patterns**. Các lập trình viên có thể triển khai những pattern ở trong hầu hết mọi ngôn ngữ. Không giống những pattern khác, chúng có thể được dùng để thiết kế kiến trúc của toàn bộ ứng dụng.

Ngoài ra, tất cả pattern có thể được phân loại theo ý định hoặc mục đích. Quyển sách này bao gồm 3 nhóm chính của pattern:
- **Creational patterns** cung cấp các cơ chế tạo ra đối tượng giúp tăng sự linh hoạt và tái sử dụng code sẵn có.
- **Structural patterns** giải thích cách lắp ráp các đối tượng và class vào các cấu trúc lớn hơn, trong khi vẫn giữ những cấu trúc đó linh hoạt và hiệu quả.
- **Behavioral patterns** quan tâm đến việc giao tiếp hiệu quả và phân công trách nhiệm giữa các đối tượng.
## Danh mục các Design Pattern
### Creational patterns
- Factory Method
- Abstract Factory
- Builder
